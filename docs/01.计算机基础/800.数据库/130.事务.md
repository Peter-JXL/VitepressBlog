---
title: 事务
date: 2023-11-24 17:29:04
permalink: /Computer/MySQLtransaction/
categories:
  - 计算机基础
  - 数据库
tags:
...
feed:
  enable: true
description: 在 DBMS 中，事务（transaction）是非常重要的概念。
---
# 事务

在 DBMS 中，事务（transaction）是非常重要的概念。
...
举例，店铺决定把运动 T 恤的销售单价下调 1000 日元，同时把 T 恤衫的销售单价上浮 1000 日元，那么使用 update 即可。

① 首先将运动 T 恤的销售单价降低 1000 日元
...
再来考虑一个例子：账户 A 给账户 B 赚钱 100，先从账户 A 减去 100，再去账户 B 增加 100，如果有一个更新失败，那么就会造成财产的损失。
...
反之，事务的结束需要用户明确地给出指示。结束事务的指令有 commit 和 rollback
‍
## COMMIT：提交处理

COMMIT 是提交事务包含的全部更新处理的结束指令。一旦提交 ，就无法恢复到事务开始前的状态了。因此，在提交之前一定要确认是否真的需要进行这些更新。

​![](https://image.peterjxl.com/blog/Image00089-20230924165636-5na50u8.jpg)​
‍

注意：一旦提交，就很难恢复到之前的数据了，就好比 delete 一样，删除后就很难恢复了。
‍
## ROLLBACK：取消处理

ROLLBACK 是取消事务包含的全部更新处理的结束指令。一旦回滚 ，数据库就会恢复到事务开始之前的状态。通常回滚并不会像提交那样造成大规模的数据损失。

​![](https://image.peterjxl.com/blog/image-20241204164028-buiv6om.png)​
‍
...
‍
注意：不能回退 select 子句（回退了也没意思），也不能回退 create 和 drop 操作，事务中可以使用这些语句，但进行回退时，这些操作也不撤销
‍
## 事务处理何时开始
...
通常的 DBMS 都可以选择其中任意一种模式。默认使用自动提交模式的 DBMS 有 SQL Server 、PostgreSQL 和 MySQL 等。 DML 语句如下所示，每一条语句都括在事务的开始语句和结束语句之中。
...
​![](https://image.peterjxl.com/blog/Image00091-20230924165852-4qtv77v.gif)​
‍
