---
title: 数据的插入
date: 2023-11-21 17:29:04
permalink: /Computer/MySQLinsert/
categories:
  - 计算机基础
  - 数据库
tags:
...
feed:
  enable: true
description: 接下来学习 DBMS 中用来更新表中数据的方法。
---
# 100.数据的插入

之前学的都是查询，所使用的 SQL 语句都是 `SELECT`​ 语句。`SELECT`​ 语句并不会更改表中数据，也就是说，`SELECT`​ 语句是读取专用的指令。

接下来学习 DBMS 中用来更新表中数据的方法。数据的更新处理大体可以分为插入（`INSERT`​）、删除（`DELETE`​）和更新（`UPDATE`​）三类。
...
我们新创建一个名为 `ProductIns`​ 的表，用来学习 insert。
...
该表和 Product 表几乎相同，除了 `DEFAULT 0`​ 的约束：这个约束的含义将会在随后进行介绍
...
* 值清单 →('0001', 'T 恤衫', '衣服', 1000, 500,'2009-09-20')
‍
注意：
...
我们还可以向表中插入默认值 （初始值）。可以通过在创建表的 CREATE TABLE 语句中设置 DEFAULT 约束 来设定默认值，例如本文开头创建的 ProductIns 表，就为 sale_price 设置了默认值 0：
