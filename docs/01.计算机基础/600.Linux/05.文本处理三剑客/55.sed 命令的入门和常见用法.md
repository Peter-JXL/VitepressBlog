---
title: sed 命令的入门和常见用法
date: 2022-11-30 21:28:39
permalink: /Computer/Linux/sed/
categories:
  - 计算机基础
  - Linux
tags:
  - 
coverImg: https://image.peterjxl.com/blog/sed_linux_en-20260129203245-3s5kwg1.png
feed:
  enable: true
description: sed 是文本处理三剑客之一，掌握它能能很有效地提高我们的效率
---
...
![Linux-grep-sed-awk](https://image.peterjxl.com/blog/Linux-grep-sed-awk-20260129203341-rk9dari.jpg)
...
- 先将 sed 指令保存到文件中，将该文件作为参数进行调用，  `sed + 选项 -f 包含 sed 指令的文件 文件`
...
<mark>前排提醒，例子有点多，不一定要全部一次性看完，建议看前面 2~3 个示范，懂个大概就行了。用到时再来看也不迟。</mark>
...
```bash
sed 'lahello' test.txt  # 
```

其中，`'lahello'` 中的数字 1 表示行号，a 表示添加 add，所以该命令的作用就是：向第一行后面添加 hello。
...
```bash
sed '/22/achina' test.txt   # 向文件内容为 22 的下一行添加 china，如果文件中有多行的内容为 22，则每一行后面都会添加
```
...
```bash
sed '5cfive' test.txt  # 将第 5 行的替换为 five
```
...
```bash
sed '1~2d' test.txt  	#从第一行开始删除，每隔两行删掉一个，即删掉奇数行
```
...
```bash
sed '1,2!d' test.txt	#删除除了 1-2 行之外的行
```
...
```bash
sed '/22/,+1d' test.txt  #删除匹配 22 的行以及下一行
```
...
```shell
sed '/22/,$d' test.txt  #删除从匹配到 22 的行到最后一行
```
...
```bash
sed '/22\|^$/!d' test.txt  # 删除不匹配 22 或者空行的行。 先匹配 22 或空行， 然后！表示取反
```
...
sed -n '3,$p' test.txt	    #打印从第 3 行到最后一行的内容
```
...
注意，如果指定的行数超过了原本的文件的行数，插入不会生效。例如 1.txt 只有 2 行，如果要在第三行后插入文件 2.txt 的内容，不会生效：
