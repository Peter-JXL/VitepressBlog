...
feed:
  enable: true
description: 该篇非常重要，讲的是如何将项目，部署到 Docker 上并运行，和我们的工作息息相关。
---
# Docker 微服务实战

该篇非常重要，讲的是如何将项目，部署到 Docker 上并运行，和我们的工作息息相关。

本节课要做的事：用 IDEA 新建一个普通的微服务项目，然后用 Dockerfile 发布微服务到 Docker 容器内

‍‍<!-- more -->

## 新建项目

这里就不再详细的演示如何新建项目了，这些在 [SpringBoot](https://www.peterjxl.com/SpringBoot/) 教程里都讲过了。读者也可以去 [Gitee](https://gitee.com/peterjxl/LearnDocker) 或 [GitHub](https://github.com/Peter-JXL/LearnDocker) 上下载本项目（分支为 demo1）

新建项目后，在 application.properties 修改端口：
...
在/mydocker 目录内新建 Dockerfile 文件：
