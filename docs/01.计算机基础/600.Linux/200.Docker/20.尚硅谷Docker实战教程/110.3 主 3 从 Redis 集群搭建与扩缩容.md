...
feed:
  enable: true
description: 讲了理论，接下来就来搭建 Redis 集群了，然后演示扩缩容.
---
...
* ​`--net host`​：使用宿主机的 IP 和端口，后续再详细说明
...
那怎么解决呢？首先问题产生的原因：目前是使用了 Redis 集群，连接也应该是连接整个集群，而不是某个结点。因此我们可以在连接的时候，加一个 `-c` ​参数，表明是以集群（cluster）的方法连接，防止路由失效：
...
1. 先删除从节点
2. 重新分配槽位（这里为了方便， 将 6387 的槽位全部分给 1 个节点）
3. 再删除主节点
‍
