...
feed:
  enable: true
description: Docker-Compose 是 Docker 公司推出的一个开源工具软件，可以管理多个 Docker 容器组成一个应用。用户需要定义一个 YAML 格式的配置文件 docker-compose.yml，写好多个容器之间的调用关系。然后，只要一个命令，就能同时启动/关闭这些容器（或者说能实现对 Docker 容器集群的快速管理，编排）
---
# 170.Docker-compose 容器编排
...
官网文档：[docs.docker.com/compose](https://docs.docker.com/compose)
...
4. 本项目主要参考老师的源码 + B 站评论区-[我自己 1213812138](https://space.bilibili.com/189545719) 的源码
...
用 `mvn package` ​打包，上传到/mydocker 目录下，并重命名为 docker_boot-0.0.1-SNAPSHOT.jar
‍
### 编写 Dockerfile
‍
