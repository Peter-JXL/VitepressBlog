...
‍
1. 在类上加注解
2. 这里我们不再继承 JdbcDaoSupport，而是自己注入 JdbcTemplate 对象
3. 方法中，不再调用父类的 get 方法获取 JdbcTemplate 对象
...
然后在需要使用事务支持的地方使用 `@Transactional` 注解，也就是 service 实现类：
...
本项目已将源码上传到 [GitHub](https://github.com/Peter-JXL/LearnSpring) 和 [Gitee](https://gitee.com/peterjxl/LearnSpring) 上。并且创建了分支 demo19，读者可以通过切换分支来查看本文的示例代码
