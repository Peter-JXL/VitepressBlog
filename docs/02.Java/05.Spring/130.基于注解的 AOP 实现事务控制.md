...
在 service 实现类中，加上@Service 的注解，并给 dao 加上 `@Autowired` 注解，去掉 set 方法
...
dao 实现类同理，加上@Repository 注解，并给成员变量加上 `@Autowired` 注解，去掉 set 方法
...
1. 给类加上 Component 的注解，和@Aspect 的注解
2. 配置切入点表达式
3. 给各个方法配置通知
...
本项目已将源码上传到 [GitHub](https://github.com/Peter-JXL/LearnSpring) 和 [Gitee](https://gitee.com/peterjxl/LearnSpring) 上。并且创建了分支 demo16，读者可以通过切换分支来查看本文的示例代码
