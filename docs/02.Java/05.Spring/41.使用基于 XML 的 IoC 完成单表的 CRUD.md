---
title: 使用基于 XML 的 IoC 完成单表的 CRUD
...
---
# 41.使用基于 XML 的 IoC 完成单表的 CRUD

对 IoC 有基本的认知后，我们来试着完成对单表的增删改查（分别使用 XML 和注解的方式），在实际运用中使用 Spring。
<!-- more -->

## 环境准备

为了方便，我们引用一些依赖 dbutils、c3p0，Junit
...
```
‍
然后我们可以删除我们之前所有的 Java 代码，只保留 pom.xml：
...
```
‍
然后我们创建 service 实现类：注意有个 IAccountDao 变量，并且有个 set 方法，后期由 Spring 帮我们注入
...
注意有个 `QueryRunner` 变量，并且有个 set 方法，后期由 Spring 帮我们注入
...
本项目已将源码上传到 [GitHub](https://github.com/Peter-JXL/LearnSpring) 和 [Gitee](https://gitee.com/peterjxl/LearnSpring) 上。并且创建了分支 demo5，读者可以通过切换分支来查看本文的示例代码。
