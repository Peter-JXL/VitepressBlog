...
1. 客户浏览器使用的编码是自己系统本身的编码，例如在国内，经常是 GBK；
2. 而服务器使用的是国外的编码（Tomcat 是国外的），`PrintWriter pw = response.getWriter();` 获取的流的默认编码是 ISO-8859-1；
3. 最后解码失败，导致乱码


解决方法：

1. 获取流之前，设置 response 的默认编码
