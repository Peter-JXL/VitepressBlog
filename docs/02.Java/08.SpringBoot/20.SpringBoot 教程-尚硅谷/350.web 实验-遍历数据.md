...
feed:
  enable: true
description: 接下来改一下小的功能以及表格的遍历功能
...
我们左侧菜单中，`Dashboard`（可以理解为是）默认还是跳转到 index.html：
...
接下来修改 dynamic_table.html 文件，将下方的那个表格中 `tbody` 标签的内容删除（从 522 行到 900 多行）；
‍
然后通过 Thymeleaf 的 [遍历语法](https://www.thymeleaf.org/doc/tutorials/3.0/usingthymeleaf.html#iteration)，来输出数据：
...
已将本文源码上传到 [Gitee](https://gitee.com/peterjxl/LearnSpringBoot-Web-Admin) 或 [GitHub](https://github.com/Peter-JXL/LearnSpringBoot-Web-Admin) 的分支 demo3，读者可以通过切换分支来查看本文的示例代码
