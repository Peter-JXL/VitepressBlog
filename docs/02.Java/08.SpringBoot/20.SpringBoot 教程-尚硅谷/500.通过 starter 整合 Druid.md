...
feed:
  enable: true
description: 本文讲讲如何通过 starter 的方式，来整合 Druid
...
在 [GitHub](https://github.com/alibaba/druid) 里，已经有一个 starter 的目录，我们点下面那个：
...
* `@ConditionalOnClass({DruidDataSource.class})` 说明得有 Druid 的 DataSource，才会生效
* `@AutoConfigureBefore({DataSourceAutoConfiguration.class})` 说明得在 SpringBoot 自动配置数据源之前，配置，目的是在官方的数据源之前，注入 Druid 自己的数据源
...
已将本文源码上传到 [Gitee](https://gitee.com/peterjxl/LearnSpringBoot-Web-Admin) 或 [GitHub](https://github.com/Peter-JXL/LearnSpringBoot-Web-Admin) 的分支 demo11，读者可以通过切换分支来查看本文的示例代码
