...
feed:
  enable: true
description: 之前我们说过，SpringBoot 内置了 Tomcat，只需启动应用即可监听端口，本文就来讲讲其原理
...
> 注意，`Undertow` 不支持 JSP
‍
这些工厂是 SpringBoot 底层默认有的，在配置类 `ServletWebServerFactoryAutoConfiguration` 中配置；直接由自动配置类帮我们自动配置好
