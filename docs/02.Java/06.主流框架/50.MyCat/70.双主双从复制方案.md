...
（4）MHA（一主一从一备）(m 是主，s 是从，b 是备，READ\_ONLY 判断主)
...
```
‍
（5）MGR（一主一从一备）(m 是主, s 是从, b 是备, READ\_ONLY 判断主)
...
（6）GARELA\_CLUSTER（一主一从，一备，m 是主，s 是从，b 多主）
...
‍
解决方法：文件 `/var/lib/mysql/auto.cnf` 里存储了 UUID，我们修改里面的内容，和另一个 UUID 不同即可。
