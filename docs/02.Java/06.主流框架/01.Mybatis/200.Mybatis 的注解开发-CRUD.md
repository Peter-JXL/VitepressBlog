...
feed:
  enable: true
description: 使用注解完成 CRUD
---
# 200.Mybatis 的注解开发-CRUD

之前我们都是使用 XML 配置文件的方式来开发，虽然之前简单讲解过一点注解的开发，但是不多；本文就来将我们之前用的 XML 配置文件，全部替换成注解。
<!-- more -->
注意：注解 和 XML 不能混用，有注解就不能用 XML，会报错。

## 环境搭建

我们将除了 pom.xml 文件之外的文件都删掉，重新来搭建一次环境：
...
所有代码已上传到了 [GitHub](https://github.com/Peter-JXL/LearnMybatis) 和 [Gitee](https://gitee.com/peterjxl/LearnMybatis) 上，并且创建了分支 demo19，读者可以通过切换分支来查看本文的示例代码。
