...
feed:
  enable: true
description: 讲讲用 Runnable 接口来创建线程，以及它和 Thread 的区别
---
...
4. 创建 Thread 对象，在构造方法中传递 Runnable 接口的实现类对象
5. 调用 start 方法
...
3. 增加程序的健壮性，实现解耦操作，代码可以被多个线程共享，代码和线程独立。我们可以传递不同的 Runnable 实现类，从而执行不同的代码
