...
feed:
  enable: true
description: 如果要进行高精度浮点值的计算，可以用 java.math 包中的 BigDecimal 类。
...
什么时候我们需要用到高精度的浮点数运算呢？当处理金额的时候，绝对不能有精度损失；而之前我们说过，double 和 float 是不精确的（参考我写的 [Java 中的浮点数](https://www.peterjxl.com/JavaSE/Double-Accurate)）
