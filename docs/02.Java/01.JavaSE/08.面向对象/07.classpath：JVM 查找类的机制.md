...
feed:
  enable: true
description: 对于刚跨入 Java 门槛的初学者来说，编译和运行一个简单的类或许很简单，但如果涉及到多个类、类存放的路径不一样、涉及到 jar 包的时候，就可能稍微复杂了。这里的难点在于对 classpath 的理解和设置上。本文通过理论与实践相结合的方式，逐步解决编译过程中各种 classpath 的方式，并有相关的练习和代码。通过本文，读者应该能在不借助 IDE 的情况下编译和运行代码。
...
1. 操作系统如何寻找 `javac` 和  `java` 命令？ 答：由于 `javac` 和  `java` 命令是可执行文件，操作系统会根据我们安装 Java 的时候配置的 path 去寻找这两个文件；
2. 操作系统如何寻找 Hello.java 和 Hello.class 文件？此时，就不是根据我们安装 Java 时的 path 变量去寻找了，到底如何查找呢？我们一步步测试并分析。
...
明明 `Hello/Hello.class` 是一个 class 文件，为什么就找不到了呢？

<span data-type="text" style="color: var(--b3-font-color13);">原来，Java 对待</span>`.java`​<span data-type="text" style="color: var(--b3-font-color13);">文件与</span>`.class`​<span data-type="text" style="color: var(--b3-font-color13);">文件是有区别的。对.java 文件可以直接指定路径给它，而 java 命令所需的.class 文件不能出现扩展名，也不能指定额外的路径给它。请牢记这一点。</span>

那么，对于 Java 所需的 `.class` 文件，如何指定路径呢？<span data-type="text" style="color: var(--b3-font-color13);">必须通过 classpath 来指定。</span>
...
JDK 发现有 classpath 选项后，就会先根据此 classpath 指定的路径寻找 `.class` 文件，找不到再去全局的 classpath 环境变量中寻找。我们可以试着删除环境变量，打开一个新的 CMD，然后再次执行上述命令。
...
> ```
>
> 1.特别注意的是，JDK 1.5 之前，javac 命令不能用 cp 来代替 classpath，而只能用 classpath
>
