...
6. 如果在调用方法链中找不到异常处理器，Java 程序**会终止**。
...
2. 注意父类的 catch 块，要出现在子类的 catch 块之前，否则会编译报错。例如 `RuntimeException`是 `Exception`的子类，必须先捕获 `RuntimeException`，再捕获 `Exception`。这很好理解，如果父类现在前面，那么子类的异常是永远捕获不到的
