...
feed:
  enable: true
description: Decorator 模式，也叫装饰器模式，是为了解决子类数量爆炸的一种设计模式，也叫 Fileter 模式。
