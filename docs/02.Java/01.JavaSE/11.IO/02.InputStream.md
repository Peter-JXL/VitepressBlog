...
feed:
  enable: true
description: InputStream 是 Java 标准库提供的最基本的输入流，它是一个抽象类，是所有输入流的超类。

---

# InputStream

`InputStream` 是 Java 标准库提供的最基本的输入流，它是一个抽象类，是所有输入流的超类。
...
用 `try ... finally` 看起来代码有点复复杂，Java7 引入了 `try(resource)` 的语法，只需要编写 `try` 语句，让编译器自动为我们关闭资源：
...
之前我们说过 `InputStream` 是同步 IO，因此在调用其相关方法的时候，我们说这些方法是阻塞（Blocking）的。
