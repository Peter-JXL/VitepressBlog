...
feed:
  enable: true
description: 本文我们演示下如何用 JDBC 连接、查询和更新数据库
---
...
2. 第二步，执行 `Statement` 对象提供的 `executeQuery("SELECT * FROM students")` 并传入 SQL 语句，执行查询并获得返回的结果集，使用 `ResultSet` 来引用这个结果集；
...
1. 确定 JDBC 连接字符串
2. 使用 DriverManager 类创建 Connection 对象
3. 创建 Statement 对象
4. 配置要执行的 SQL，并使用 `executeQuery` 方法来执行 SQL
