...
feed:
  enable: true
description: 在 Java1.4，Java 标准库内部就提供了这个工具类， java.util.logging
---
# JDK Logging
...
可以看到，虽然我们没设置输出打印的时间，调用类和执行方法是什么，但都自动打印了。为什么 `logger.fine()` 没有打印？因为 fine 的级别比较低。日志的输出可以设定级别。JDK 的 Logging 定义了 7 个日志级别，从严重到普通：
...
* 1$ 日期时间  
  ...
  tL 毫秒
* 2$ 输出日志的类名和方法名
* 3$ Logger's name. 即 getLogger(name) 中的 name
* 4$ log level
* 5$ 输出内容
‍
注意：在 VSCode 的终端里运行的时候，一直报错：
...
[使用 JDK Logging - 廖雪峰的官方网站](https://www.liaoxuefeng.com/wiki/1252599548343744/1264738568571776)

[java 底层 JDK Logging 日志模块处理细节深入分析_java_脚本之家](https://www.jb51.net/article/242105.htm)
