...
在官网下载 jar 包  [Log4j – Download Apache Log4j™ 2](https://logging.apache.org/log4j/2.x/download.html)
...
api 的 jar 包是接口规范，core 的 jar 包才是具体的实现，如果忘了可以回顾下：[常见的日志框架-简单介绍](https://www.peterjxl.com/JAVAEE/log-popular-framework/)
...
可以看到输出了 error，warn 和 INFO。并且当前目录下生成了 log 文件夹，里面有 err.log 文件。
...
可以看到生成了 err2.log 日志文件，也就是采用了我们指定的配置文件
...
```
‍
如果还找不到上面的文件，则在 classpath 路径中按照顺序查找如下 4 个文件：
...
* 控制台 ConsoleAppender、
* 文件 FileAppender、AsyncAppender、RandomAccessFileAppender、RollingFileAppender、RollingRandomAccessFile 等。
...
[Log4j2 基本使用入门 - 简书](https://www.jianshu.com/p/a344409cf08a)

[log4j2 入门教程_timchen525 的博客-CSDN 博客](https://blog.csdn.net/timchen525/article/details/77844534)

[log4j2 配置详细 - gsluofu - 博客园](https://www.cnblogs.com/leifonlyone/p/15056075.html)
