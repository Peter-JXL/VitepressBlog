...
feed:
  enable: true
description: 可以在 IDE 中对 Maven 进行一些设置，更加方便我们的开发
---
...
打开 [网址](https://repo1.maven.org/maven2/) 后，按 `Ctrl+F` 搜索文件名 archetype-catalog.xml，可以找到该文件。文件约为 13M，下载速度很慢（国外），导致创建 Maven 工程的时候很慢；

![](https://image.peterjxl.com/blog/image-20230415223836-cii4c2o.png)
‍

而我们可以配置 `-DarchetypeCatalog=internal`，不联网下载，而是从本地获取骨架。这样只要曾经联网下载过骨架，后续创建 Maven 项目都会很快了。
...
参考：[idea 手动创建 webapp（在 main 文件夹下）_idea webapp_没反应说说说的博客-CSDN 博客](https://blog.csdn.net/mfysss/article/details/128101846)
‍
## 重新构建工件
